...
std::string DexDumper::nFolder = "";
// =============== DexDumper ====================

DexDumper::DexDumper(JNIEnv *env, DvmDex *dvmDex, jobject upkObj, std::string mRoot) {
    mEnv = env;
    mDvmDex = dvmDex;
    mUpkObj = upkObj;
    nFolder = mRoot;
            ...
            if (Clazz != nullptr) {
                ...
                for(int i = 0; i < newData->header.directMethodsSize; i++) {
                    fixMethodByDvm(shared, &newData->directMethods[i],
                                   &defBuilder, lastIndex, descriptor);
                }
                lastIndex = 0;
...
bool DexDumper::solveMethod(Method *meth, const char *classname) {
    if (strstr(classname, "Landroid") != NULL)
        return false;

    const DexProto *pDexProto = &(meth->prototype);

    FLOGI("solveMethod %s.%s", classname, meth->name);


    if (pDexProto == NULL) {
        FLOGI("pDexProto = NULL");
        return false;
    }

    const DexFile *pDexFile = pDexProto->dexFile;
    ...
    const DexProtoId *pDexProtoId = &(pDexFile->pProtoIds[pDexProto->protoIdx]);
    const DexTypeList *pDexTypeList = dexGetProtoParameters(pDexFile, pDexProtoId);

    //class name : classname
    //method name : meth->name
    //argument class : dexStringByTypeIdx(pDexFile, pDexTypeList->list[i].typeIdx)

    jclass clazz = mEnv->FindClass("android/app/fupk3/dumpMethod");
    if (clazz == NULL) {
        FLOGI("dumpMethod not found!");
        return false;
    }

    jmethodID static_method = mEnv->GetStaticMethodID(clazz,
                                                      "hookMethod",
                                                      "(Ljava/lang/String;Ljava/lang/String;[Ljava/lang/String;Landroid/app/fupk3/Fupk;)Z");

    if (static_method == NULL) {
        FLOGI("hookMethod not found!");
        return false;
    }
    ...
    if (pDexTypeList == NULL)
        ...
        cnt = pDexTypeList->size;

    jclass Stringclazz = mEnv->FindClass("java/lang/String");
    jstring emptystring = mEnv->NewStringUTF("");
    jobjectArray pareTyple = (jobjectArray)mEnv->NewObjectArray(cnt,
                                                                Stringclazz,
                                                                emptystring);
    mEnv->DeleteLocalRef(Stringclazz);
    mEnv->DeleteLocalRef(emptystring);

    for (int i = 0; i < cnt; i++) {
        jstring tmp = mEnv->NewStringUTF(dexStringByTypeIdx(pDexFile, pDexTypeList->list[i].typeIdx));
        mEnv->SetObjectArrayElement(pareTyple,
                                    i,
                                    tmp);
        mEnv->DeleteLocalRef(tmp);
    }
    jstring sclassname = mEnv->NewStringUTF(classname);
    jstring smethname = mEnv->NewStringUTF(meth->name);
    jboolean flag = mEnv->CallStaticBooleanMethod(clazz,
                                                  static_method,
                                                  sclassname,
                                                  smethname,
                                                  pareTyple,
                                                  mUpkObj);
    mEnv->DeleteLocalRef(sclassname);
    mEnv->DeleteLocalRef(smethname);
    mEnv->DeleteLocalRef(pareTyple);
...
bool DexDumper::fixMethodByDvm(DexSharedData &shared, DexMethod *dexMethod,
     ...
     * 反射调用时还用原来的那一套接住
     */
    if (DexDumper::solveMethod((Method *)m, classname) == false)
...
bool fupk_ExportMethod(void *thread, Method *method) {
    FLOGI("class_name : %s", method->clazz->descriptor);
    FLOGI("method_name : %s", method->name);
    ///data/local/tmp/ +
    if (strstr(method->clazz->descriptor, "Landroid") == NULL ||
        (strstr(method->clazz->descriptor, "Landroid") - method->clazz->descriptor) != 0) {

        std::string itfolder = DexDumper::nFolder + "/" + std::string(method->clazz->descriptor);
        itfolder[itfolder.length() - 1] = '/';

        std::string nfolder = "/";
        for (int i = 1; i < itfolder.length(); i++) {

            nfolder.push_back(itfolder[i]);
            if (itfolder[i] == '/')
                mkdir(nfolder.c_str(), 0700);
        }
        FILE *pfile = fopen((nfolder + std::string(method->name) + std::string(method->shorty)).c_str(), "wb");

        u2 insns[10010];
        int insSize = 0;

        if (method->insns != nullptr)
            insSize = method->insSize;
        for (int i = 0; i < insSize; i++) {
            insns[i] = method->insns[i];
        }
        fwrite(insns, sizeof(u2), insSize, pfile);
